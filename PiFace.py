...
def P0():	# Process 0 controlles servo0
	...
	_Servo0CP = 100		# by making the current position and desired position unequal,-
...
while True:
	...
	if Cface[0] != 0:		# if the Center of the face is not zero (meaning no face was found)
			...
			#CamRight(5,1)	#	from the center of that axis it detects a face
			#GPIO.setmode(GPIO.BOARD)
		...
		if Cface[1] < 100:	# and moves diffrent servos depending on what axis we are talking about.
			p = GPIO.PWM(7, 50)
			p.start(10)
			try:
